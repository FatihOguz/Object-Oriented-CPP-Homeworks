...
Course Lecturer::assignCourse(vector <Course> courses ,Course temp,int courses_size)
{
	int i,count=0;
...
Course Lecturer::proposeCourse(vector <Course> courses,Course temp,int courses_size)
{
	int count1=0;
	int i=0;
	for(i=0;i<courses_size;i++)
...
void initialized_courses_and_classrooms(Course courses[],int courses_size,Classroom classrooms[],int classrooms_size)
{
	...
	inputStream>>arr;
		...
		if(arr == "COURSES")									/* initilzed courses part */
		{
			...
			{
				inputStream>>finteger;		/* number in file*/
				classrooms[classrooms_index].id=finteger;
				inputStream>>arr;
				classrooms[classrooms_index].classNo=arr;
				inputStream>>finteger;
				classrooms[classrooms_index].capacity=finteger;
			}
		}
