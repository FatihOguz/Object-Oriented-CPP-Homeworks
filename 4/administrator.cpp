...
namespace Administratort
{
	...
	Administrator&  Administrator::operator=(Administrator& other )
	{
		...
		set_password(other.get_password());
		return *this;
	}
	...
	Administrator::Administrator(Administrator& admin):pid(admin.get_pid()),password(admin.get_password())
	{
		//cout<<"Called Copy Constructor"<<endl;
	}


	Course Administrator::arrangeTimeTable(Course courses[],int courses_id,int courses_size)
	{
		int i=0;
		int count=0;
		int day;
		...
		if(courses_id >= courses_size)			/* control course */
		{
	...
	Classroom& Administrator::arrangeClassroom(Classroom classrooms[],int classrooms_size,Course courses[],int courses_size,Classroom crm, Course crs)
	{
		int i=0,j=0,k=0;
		...
		{
			if(classrooms[i].get_id()==crm.get_id())
			{
		...
		if(flag1==0 || flag2==0 || flag3==0)
		{
			...
			return temp[0]; /* classroom's of flag 0 DONE if do 1 !!!!!!!!!!!*/
		}
		...
		{	
			...
			temp[0]=crm;
			temp[0].set_flag(1);		/* courses's of flag 0 DONE if do 1 !!!!!!!!!!!*/
			return temp[0];
		...
		else
		{
			...
			return temp[0];	/* classroom's flag  0 if  do  1 !!!!!!(DONE) */
		}
	}
