...
int main()
{
	...
	int student_size=0;
	read_students(student_size);

	Student * StudentPtr = new Student[student_size];

	initialized_student(&StudentPtr[0],student_size);
	...
	classrooms = new Classroom[classrooms_size];
	


	Course * courses;
	courses = new Course[courses_size];

	Lecturer * lecturers;
	lecturers = new Lecturer[lecturers_size];
...
test( classrooms,classrooms_size,courses, courses_size,lecturers,lecturers_size,administrator,StudentPtr,student_size);
	
	/* delete dynamic array and new dynamic array */
	delete [] courses;
	delete [] lecturers;
	delete [] classrooms;
	delete [] StudentPtr;
	...
	StudentPtr = new Student[student_size];
	...
	for(int i=0;i<classrooms_size;i++)
	{
		for(int j=0;j<9;j++)
		{
			classrooms[i].available->pzt[j]=0;
			classrooms[i].available->sal[j]=0;
			classrooms[i].available->car[j]=0;
			classrooms[i].available->per[j]=0;
			classrooms[i].available->cum[j]=0;
		}		
	}
		...
		else if(command=="-arrangeC")			/* command arrangeC */
		{
			...
			for(i=0;i<courses_size;i++)
			{
				...
				if(temp[0].get_flag()==1)			/* print courses and course date done tmetable*/
				{
					...
					outputStream<<"DONE: ("<<crs.id+1<<") in "<<crm.get_id()+1<<" at "<<courses[crs.id].sDate1<<" "<<courses[crs.id].sHourStart1
						<<endl;
				}
			}
			cout<<endl;
